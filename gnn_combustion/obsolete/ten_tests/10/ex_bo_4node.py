...
def create_mini_dataset(num_samples=1000):
    ...
    sample_types = [
        # Format: (Name, Atoms, Bonded Pairs, Internal Non-Bonded Distances)
        ('H2 + O2', ['H', 'H', 'O', 'O'], [(0, 1), (2, 3)], {}), 
        ('H2 + O + O', ['H', 'H', 'O', 'O'], [(0, 1)], {}),
        ('O2 + H + H', ['O', 'O', 'H', 'H'], [(0, 1)], {}),
        ('H2O + O', ['H', 'H', 'O', 'O'], [(0, 2), (1, 2)], {
            (0, 1): 1.51  # H-H distance in water
        }),
        ('OH + OH', ['O', 'H', 'O', 'H'], [(0, 1), (2, 3)], {}),
        ('OH + O + H', ['O', 'H', 'O', 'H'], [(0, 1)], {}),
        ('H2O2', ['H', 'O', 'O', 'H'], [(0, 1), (1, 2), (2, 3)], {
            (0, 2): 1.85, # H...O 1,3-interaction
            (1, 3): 1.85, # O...H 1,3-interaction
            (0, 3): 2.50  # H...H 1,4-interaction
        }),
        ('HO2 + H', ['H', 'O', 'O', 'H'], [(0, 1), (1, 2)], {
            (0, 2): 1.95  # H...O 1,3-interaction in HO2
        }),  
        ('H + H + O + O', ['H', 'H', 'O', 'O'], [], {}) 
    ]
    ...
    for _ in range(num_samples):
        ...
        dist_map = {}
        # Get the internal distances for the current template
        _, base_atom_types, base_bonded_pairs, base_internal_specs = random.choice(sample_types)
        ...
        # Map the internal distance indices to the new permutation
        internal_specs = {tuple(sorted((index_map[u], index_map[v]))): d 
                  for (u, v), d in base_internal_specs.items()}
        ...
        # Map internal specs to new indices
        internal_specs = {(index_map[u], index_map[v]): d for (u, v), d in base_internal_specs.items()}     
        ...
        for i in range(12):
            ...
            if pair_key not in dist_map:
                if (u, v) in bonded_pairs or (v, u) in bonded_pairs:
                    # Keep your existing randomization for actual bonds
                    dist_map[pair_key] = random.uniform(0.7, 1.6)
                elif pair_key in internal_specs:
                    # Use the specific internal distance (e.g., 1.51 for H-H in water)
                    # Add a small amount of noise if you want robustness
                    d_base = internal_specs[pair_key]
                    dist_map[pair_key] = random.uniform(d_base - 0.05, d_base + 0.05)
                else:
                    # Truly non-bonded/separate species
        ...
        data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y_bond=y_bond, y_exist=y_exist)
        dataset.append(data)
...
def train():
    ...
    for batch in train_loader:
        ...
        # Can add weights to balance the two losses if needed
        loss = loss_bond + loss_exist
        
        loss.backward()
        optimizer.step()

        total_loss += loss.item()
        total_loss_bond += loss_bond.item()
        total_loss_exist += loss_exist.item()

    n_train =len(train_loader)
    return total_loss/n_train, total_loss_bond/n_train, total_loss_exist/n_train
...
for epoch in range(1, 201):
    avg_total, avg_bond, avg_exist = train()
    if epoch % 10 == 0 or epoch == 1:
        print(f"Epoch: {epoch:03d} | Total loss: {avg_total:.4f} | Bond loss: {avg_bond:.4f} | Existence loss: {avg_exist:.4f}")
...
with torch.no_grad():
    cases = [
        ...
        ('H2O + O', [0, 0, 1, 1], {
            (0, 2): 0.97, (1, 2): 0.97, # O-H bonds
            (0, 1): 1.51,               # H-H distance in H2O
            (0, 3): 3.50, (1, 3): 3.50, (2, 3): 3.50 # Lone O spacing
        }),
        ('OH + OH', [0, 1, 0, 1], {(0, 1): 0.97, (2, 3): 0.97}),
        ('H2O2', [0, 1, 1, 0], {
            (0, 1): 0.97, (1, 2): 1.48, (2, 3): 0.97, # H-O-O-H chain
            (0, 2): 1.85, (1, 3): 1.85,               # 1,3-interactions
            (0, 3): 2.50                              # H-H distance across the molecule
        }),
        ('HO2 + H', [0, 1, 1, 0], {
            (0, 1): 0.97, (1, 2): 1.33, # H-O and O-O bonds
            (0, 2): 1.85,               # Non-bonded H...O in HO2
            (0, 3): 3.50, (1, 3): 3.50, (2, 3): 3.50 # Lone H spacing
        }),
        ('H2 + O + O', [0, 0, 1, 1], {(0, 1): 0.74}),
        ('O2 + H + H', [1, 1, 0, 0], {(0, 1): 1.21}),
        ('OH + O + H', [0, 1, 1, 0], {(0, 1): 0.97}),
        ('H + H + O + O', [0, 0, 1, 1], {}),
        ('O2 (stretched) + H + H', [1, 1, 0, 0], {(0, 1): 1.70}),
        ('H2O (TS) + O', [0, 0, 1, 1], {(0, 1): 2.10, (0, 2): 0.93, (1, 2): 1.95})
    ]
...
# 6. SAVE
# ==========================================
torch.save(model.state_dict(), "ex_bo_4node.pth")
print("Model weights saved to ex_bo_4node.pth")