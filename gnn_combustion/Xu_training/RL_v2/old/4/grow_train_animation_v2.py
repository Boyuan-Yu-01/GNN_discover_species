...
class AdvancedMoleculeEnv:
    ...
    def step(self, action_class, u_global, v_global=None, new_atom_type=None, bond_order_inc=0):
        ...
        if action_class == 0:
            ...
            target_order = 2 if bond_type == 1 else 1

            # 2. Calculate how many units we are adding
            # Only add if the target order is higher than what is already there
            if target_order > current_order:
                order_increment = target_order - current_order
            #valency_cost = target_order 
            
            # 3. Only proceed if we are actually adding capacity
            if order_increment > 0:
                ...
                self.current_bonds[u_global - self.offset] += order_increment
                self.current_bonds[v_global - self.offset] += order_increment
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        while not env.terminated and steps < 8:
            ...
            if max_grow_val >= max_conn_val:
                ...
                env.step(action_class=1, u_global=u_global, v_global=v_global, bond_order_inc=bond_order)

                # Update the NetworkX graph so the visualization reflects the new bond
                global_G.add_edge(u_global, v_global)
                if bond_type == 1:
                    print(f"DEBUG: Attempting double bond between {u_local} and {v_local}")
                else:
                    print(f"DEBUG: Attempting single bond between {u_local} and {v_local}")
