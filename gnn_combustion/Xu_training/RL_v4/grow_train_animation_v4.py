...
OUTPUT_FOLDER = os.path.join(SCRIPT_FOLDER, "output1")
...
def save_restart_file(epoch, model, optimizer, reward, p_valid_stop, config):
...
class AdvancedMoleculeEnv:
    ...
    def evaluate_inventory(self):
        # Score each molecule: known multi-atom species = +1, unknown = -0.5,
        # and an isolated atom = 0. The episode reward is their average.
        unique_sets = []
        ...
        for group in unique_sets:
            ...
            if species_name is not None:
                ...
                # A recognized multi-atom molecule scores +1; an isolated atom scores 0.
                score = ISOLATED_ATOM_REWARD if len(group) == 1 else RECOGNIZED_MOLECULE_REWARD
                molecule_scores.append(score)
                score_details.append(f"{species_name}={score:+.1f}")
                continue
...
if __name__ == "__main__":
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        while not env.terminated and steps < max_steps_per_epoch:
            ...
            # term_logit: score for stopping generation
        ...
        # Probability that the GNN considers the final graph a valid stopping point.
        p_valid_stop = torch.sigmoid(final_term_logit).item()
        ...
        print(f"Epoch {epoch:02d}/{total_epochs} | System Output: {mixture_str:<22} "
              f"| Reward: {reward:+.3f} | P_valid_stop: {p_valid_stop:.3f} "
              f"| Training Loss: {total_loss.item():.5f}")

        restart_path = save_restart_file(
            epoch, gnn_model, optimizer, reward, p_valid_stop, training_config
        )
        ...
        epoch_snapshots.append({
            ...
            'text': (f"Epoch {epoch}: {mixture_str} | Reward: {reward:+.3f} "
                     f"| P_valid_stop: {p_valid_stop:.3f}")
        })
